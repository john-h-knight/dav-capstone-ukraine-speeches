...
# vector for the number of pages after filtering for Feb - June 2022
page_numbers <- 1:24

# define base url that pages numbers will be added to
base_url <- "https://www.president.gov.ua/en/news/speeches?date-from=14-02-2022&date-to=30-06-2022&page="
...
paging_urls_df <- tibble("page" = c(1:24),
                         "link" = paging_urls
                         )

# function to scrape data from each page of the list
list_5mo_info <- function(link, page) {
...
safe_list_5mo_info <- safely(list_5mo_info)
...
list_5mo_df_ALL <- map(.x = paging_urls_df$link,
                       ~ safe_list_5mo_info(link = .x))
...
page_5mo_info <- function(link_page) {
...
# wrap function for easier debugging
safe_page_5mo_info <- safely(page_5mo_info)
...
page_5mo_df_ALL <- map(.x = list_5mo_df$link,
                       ~ safe_page_5mo_info(link_page = .x))
...
# export
write_csv(page_5mo_df, file = 'data/2022_feb_jun.csv')
