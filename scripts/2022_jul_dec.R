...
# vector for the number of pages after filtering for July - Dec 2022
page_numbers <- 1:28

# define base url that pages numbers will be added to
base_url <- "https://www.president.gov.ua/en/news/speeches?date-from=01-07-2022&date-to=31-12-2022&page="
...
paging_urls_df <- tibble("page" = c(1:28),
                         "link" = paging_urls
                         )

# function to scrape data from each page of the list
list_6mo_info <- function(link, page) {
...
safe_list_6mo_info <- safely(list_6mo_info)
...
list_6mo_df_ALL <- map(.x = paging_urls_df$link,
                       ~ safe_list_6mo_info(link = .x))
...
page_6mo_info <- function(link_page) {
...
# wrap function for easier debugging
safe_page_6mo_info <- safely(page_6mo_info)
...
page_6mo_df_ALL <- map(.x = list_6mo_df$link,
                       ~ safe_page_6mo_info(link_page = .x))
...
# export
write_csv(page_6mo_df, file = 'data/2022_jul_dec.csv')
